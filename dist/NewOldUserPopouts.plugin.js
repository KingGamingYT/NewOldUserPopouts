...
function ActivityCard({ user, activity, check }) {
	...
	return BdApi.React.createElement(BdApi.React.Fragment, null, BdApi.React.createElement("h3", { className: "headerTextNormal headerText size12", style: { color: "var(--white)", marginBottom: "8px" } }, (check?.listening || check?.watching) && [2, 3].includes(activity?.type) ? headers[activity.type] + activity?.name : filterCheck?.xbox || filterCheck?.playstation ? intl.intl.formatToPlainString(intl.t["A17aM8"], { platform: activity?.platform }) : headers[activity.type]), BdApi.React.createElement("div", { className: activity?.assets ? "bodyAlignCenter" : "bodyNormal", style: { display: "flex", alignItems: "start", width: "auto" } }, BdApi.React.createElement(
		"div",
		{
			className: "assets",
...
function SpotifyCards({ user, activities }) {
	...
	)) : BdApi.React.createElement("svg", { style: { width: "60px", height: "60px" }, viewBox: "0 0 24 24" }, BdApi.React.createElement(
		"path",
		{
			fill: "white",
			"fill-rule": "evenOdd",
...
function TwitchCards({ user, activities }) {
	...
	)) : BdApi.React.createElement("svg", { style: { width: "40px", height: "40px" }, viewBox: "0 0 24 24" }, BdApi.React.createElement(
		"path",
		{
			fill: "white",
			"fill-rule": "evenOdd",
...
function HeaderInnerBuilder({ data, user, displayProfile, tagName, displayName, nickName, activities }) {
	...
	return BdApi.React.createElement("div", { className: "headerTop", style: { flex: "1 1 auto" } }, BdApi.React.createElement(AvatarFetch, { className: "avatarWrapper", user, guildId: displayProfile.guildId, themeType: "POPOUT", onOpenProfile: () => {
