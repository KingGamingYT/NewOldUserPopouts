...
const [
	...
	{ filter: betterdiscord.Webpack.Filters.byKeys("avatar", "clickable", "focusRing") },
	{ filter: betterdiscord.Webpack.Filters.byStrings("STREAMING", "isVROnline"), searchExports: true },
	{ filter: betterdiscord.Webpack.Filters.byStrings("statusBackdropColor", "voiceDb"), searchExports: true },
	...
	{ filter: betterdiscord.Webpack.Filters.byStrings('"aria-describedby"', "onDoubleClick"), searchExports: true },
...
function RoleAddButton({ guild, guildMember, highestRole, onAddRole, buttonRef }) {
	...
	return BdApi.React.createElement(
		Popout,
		{
			...
			clickTrap: true,
			onRequestClose: () => setShowPopout(false),
			renderPopout: () => BdApi.React.createElement(PopoutContainer, { position: "top" }, BdApi.React.createElement(
				RoleAddPopout,
				{
					guild,
					onSelect: onAddRole,
		...
		(props) => BdApi.React.createElement(
			...
			BdApi.React.createElement(TooltipBuilder, { note: intl.intl.formatToPlainString(intl.t["ljnBlo"]) }, BdApi.React.createElement("div", { className: "addButton" }, BdApi.React.createElement("svg", { role: "img", width: "18", height: "18", fill: "none", viewBox: "0 0 24 24" }, BdApi.React.createElement("path", { fill: "currentColor", d: "M13 6a1 1 0 1 0-2 0v5H6a1 1 0 1 0 0 2h5v5a1 1 0 1 0 2 0v-5h5a1 1 0 1 0 0-2h-5V6Z" }))))
		)
	);
}
...
function AvatarOverlay({ show, ...avatarProps }) {
	...
	return BdApi.React.createElement("svg", { width: size, height: size, viewBox: `0 0 ${size} ${size}`, style: { opacity: show && 1 }, className: `avatarHint` }, BdApi.React.createElement("foreignObject", { x: "0", y: "0", width: size, height: size, mask: `url(#svg-mask-avatar-status-round-${size})` }, BdApi.React.createElement("div", { className: "avatarHintInner" }, intl.intl.formatToPlainString(intl.t["uTre2y"]))));
}
function AvatarWithOverlay({ user, displayProfile, themeType, className, onOpenProfile }) {
	...
	const [showHoverOverlay, shouldShowHoverOverlay] = react.useState(false);
	return !onOpenProfile ? BdApi.React.createElement("div", { className: `${AvatarClasses.avatar} ${className}`, ...eventHandlers }, BdApi.React.createElement(AvatarWrapper, { ...avatarProps })) : BdApi.React.createElement(
		FocusRing,
		{
		...
		BdApi.React.createElement(AvatarWrapper, { ...avatarProps }),
		BdApi.React.createElement(AvatarOverlay, { show: showHoverOverlay, ...avatarProps })
	);
}
function HeaderInnerBuilder({ data, user, displayProfile, tagName, displayName, nickName, activities }) {
	...
	return BdApi.React.createElement("div", { className: "headerTop", style: { flex: "1 1 auto" } }, BdApi.React.createElement(AvatarWithOverlay, { className: "avatarWrapper", user, guildId: displayProfile.guildId, themeType: "POPOUT", onOpenProfile: () => {
...
const styles = Object.assign(
	{
		...
		avatarDecorationContainer: betterdiscord.Webpack.getByKeys("avatarDecorationContainer").avatarDecorationContainer
	},
	Object.getOwnPropertyDescriptors(betterdiscord.Webpack.getByKeys("container", "bar", "progress")),
	Object.getOwnPropertyDescriptors(betterdiscord.Webpack.getByKeys("outer", "overlay")),
	Object.getOwnPropertyDescriptors(betterdiscord.Webpack.getByKeys("inlineContainer", "themedBackground")),
